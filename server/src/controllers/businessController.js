...
export async function createStaff(req, res, next) { try { if (req.user.role !== 'business_owner') return res.status(403).json({ message: 'Only a business owner can manage staff' }); const owner = await User.findById(ownerIdFor(req.user)); const ownerId = owner._id; await checkLimit(ownerId, User, 'maxStaff', 'staff'); const { name, userName, emailId, password, phoneNumber, permissionLevel, subscriptionPlan, subscriptionStartDate, subscriptionEndDate } = req.body; const branchIds = normalizeBranchIds(req.body.allowedBranches); if (!name || !userName || !emailId || !password || !['view', 'edit', 'full'].includes(permissionLevel)) return res.status(400).json({ message: 'Name, username, email, password and permission level are required' }); if (!branchIds || !branchIds.length) return res.status(400).json({ message: 'Select at least one branch for this staff member' }); if (!branchIds.every((id) => mongoose.isValidObjectId(id))) return res.status(400).json({ message: 'One or more selected branches are invalid' }); const branches = await Branch.countDocuments({ _id: { $in: branchIds }, businessOwnerId: ownerId, status: 'active' }); if (branches !== branchIds.length) return res.status(400).json({ message: 'One or more selected branches are invalid' }); let staffPlan = subscriptionPlan ? subscriptionPlan.toString().trim() : owner.subscriptionPlan; if (staffPlan && staffPlan !== owner.subscriptionPlan) { const belongs = await Subscription.exists({ planName: staffPlan, planType: 'business', status: 'active' }); if (!belongs) return res.status(400).json({ message: 'Selected subscription plan is not a valid business plan for this owner' }); } let startDate = subscriptionStartDate ? new Date(subscriptionStartDate) : owner.subscriptionStartDate; let endDate = subscriptionEndDate ? new Date(subscriptionEndDate) : owner.subscriptionEndDate; if (Number.isNaN(startDate.getTime())) startDate = owner.subscriptionStartDate; if (Number.isNaN(endDate.getTime())) endDate = owner.subscriptionEndDate; const staffUser = await User.create({ name, userName, emailId: String(emailId).toLowerCase(), password: await bcrypt.hash(password, 10), phoneNumber, role: 'business_staff', businessOwnerId: ownerId, permissionLevel, allowedBranches: branchIds, approvalStatus: 'approved', subscriptionPlan: staffPlan || undefined, subscriptionStartDate: startDate || undefined, subscriptionEndDate: endDate || undefined }); try { await ActivityLog.create({ businessOwnerId: ownerId, staffUserId: staffUser._id, action: 'create_staff', module: 'staff', recordId: staffUser._id, description: `${req.user.userName} added staff member ${staffUser.name || staffUser.userName}` }); } catch { } res.status(201).json(staffUser.select('-password -refreshToken')); } catch (e) { next(e); } }
export async function updateStaff(req, res, next) { try { if (req.user.role !== 'business_owner') return res.status(403).json({ message: 'Only a business owner can manage staff' }); const ownerId = ownerIdFor(req.user); const update = {};     const { name, userName, emailId, phoneNumber, permissionLevel, password, status, subscriptionPlan, subscriptionStartDate, subscriptionEndDate } = req.body; if (name !== undefined) update.name = name; if (userName !== undefined) update.userName = userName; if (emailId !== undefined) update.emailId = String(emailId).toLowerCase(); if (phoneNumber !== undefined) update.phoneNumber = phoneNumber; if (status !== undefined) { if (!['active', 'inactive'].includes(status)) return res.status(400).json({ message: 'Invalid status' }); update.status = status; } if (permissionLevel !== undefined) { if (!['view', 'edit', 'full'].includes(permissionLevel)) return res.status(400).json({ message: 'Invalid permission level' }); update.permissionLevel = permissionLevel; } if (password) update.password = await bcrypt.hash(String(password), 10);
